...
// 1. filter : user의 나이가 30세 미만인 사람들을 출력해보세요
...
// 2. 사용자 이름만 추출해보세요. 배열로
const userNames = users.map((user) => user.name);
console.log('사용자 이름만 추출', userNames);

// 3. 나이가 25세 미만인 사람들의 이름만 출력해보세요
const youngUsersName = users.filter((user) => user.age < 25).map((user) => user.name);
console.log('나이가 25세 미만인 사람들의 이름은', youngUsersName);
