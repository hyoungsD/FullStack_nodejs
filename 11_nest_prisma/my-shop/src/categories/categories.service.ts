...
export class CategoriesService {
  ...
  async create(createCategoryDto: CreateCategoryDto) {
    ...
    if(exists) throw new ConflictException(`이미 있는 이름입니다. ${createCategoryDto.name}`);
  ...
  async update(id: number, updateCategoryDto: UpdateCategoryDto) {
    await this.findOne(id); // 위에서 findOne을 만들었기 때문에 없으면 여기에서 에러메시지 보여줌
    // postman body {}
    if(Object.keys(updateCategoryDto).length < 1){
      throw new BadRequestException(`수정할 필드가 없습니다.`)
    }
    if(updateCategoryDto !== undefined){
      const exists = await this.prisma.category.findUnique({
        where: {name: updateCategoryDto.name}
      });
      if(exists && exists.id !== id){
        throw new ConflictException(`이미 존재하는 분류입니다. ${updateCategoryDto.name}`);
      }
    }
    ...
    // update category set name = ?, created = ?, where id = ?
    // data: dto
  }

  async remove(id: number) {
    // product가 category 하위에 있을 경우 해당 제품의 존재여부 체크 로직 들어가야 함
